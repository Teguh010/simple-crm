...
const exportCSV = () => {
  ...
  const fileName = `category_${formattedSelectedDayRange}_${clinicStore.getClinic.name_clinic_display}(出力${formattedDate}).csv`;
    ...
    <q-card-section class="q-px-lg content" style="overflow-x: scroll;">
      <div class="header-info">
        <div class="left">
          ...
          <span class="q-ml-md">{{ props.params.date_start + ' ~ ' + props.params.date_end }}</span>
        </div>
        <div class="right">
            ...
            <template v-for="category1 in itemType.children" :key="category1.label">
              ...
              <template v-for="category2 in category1.children" :key="category2.code_category2">
                <tr :class="'heading' + category2.header">
                  ...
                  <td class="text-right">{{ category2.average }}</td>
