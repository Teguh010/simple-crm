...
import UpdateCommonLabDeviceModal from './UpdateCommonLabDeviceModal.vue'
...
const onRowClick = async (row: any) => {
  await mtUtils.smallPopup(UpdateCommonLabDeviceModal, {
    commonObj: row,
...
const init = async () => {
  ...
  if (response) {
    commonOptionList.value = commonStore.getCommonDeviceOptionActiveList
  }
}
