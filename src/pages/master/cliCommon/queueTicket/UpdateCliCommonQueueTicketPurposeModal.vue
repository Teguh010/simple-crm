...
import { typeUnitBit, typeOccupation } from '@/utils/enum'
...
onMounted(() => {
  ...
  if (props.commonObj && props.commonObj.id_cli_common) {
    ...
    if (props.commonObj.code_cli_common == 4) {
      ...
      selectedTypeOccupations.value = typeOcupationNew.value.filter(i => props.commonObj.text1.split(',').includes(i.value.toString())) 
    }
  }
