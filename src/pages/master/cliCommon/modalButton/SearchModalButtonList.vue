...
import UpdateCliCommonModalButtonModal from './UpdateCliCommonModalButtonModal.vue'
...
const openAddModal = async (params = null) => {
  await mtUtils.smallPopup(UpdateCliCommonModalButtonModal)
  init()
}
const onRowClick = async (row: any) => {
  ...
  await mtUtils.smallPopup(UpdateCliCommonModalButtonModal, { commonObj: propsData })
  init()
}
...
const showItemService = (value: number) => itemStore.getAllServiceItems.find((item) => item.id_item_service == value)?.name_item_service
const showItemServiceUnit = (value: number) => itemServiceUnitStore.getItemServiceUnits?.find((item) => item.id_item_service_unit == value)?.name_service_item_unit

const init = async () => {
  await Promise.all([
    printStore.fetchPrints(),
    cliCommonStore.fetchPreparationCliCommonList({code_cli_common: [18]}, true),
  ...
  <q-page :style="{ 'min-height': 'unset !important' }">
    ...
    <div class="row items-center justify-between q-px-lg tableBox">
      <div class="body1 regular text-grey-700">
        表示：<span class="q-ml-sm"> {{ cliCommonStore.getCliCommonModalButton.length }} 件</span>
      </div>
    ...
    <q-table
      :columns="columns"
      :rows="cliCommonStore.getCliCommonModalButton"
      :rowsBg="true"
      ...
      <template v-slot:body="props">
        <q-tr :props="props" class="cursor-pointer">
          ...
          >
            ...
            <div v-else-if="col.field === 'memo_etc2'" class="body1 regular text-grey-900">
