...
import UpdateCliCommonPublicHealthModal from './UpdateCliCommonPublicHealthModal.vue'
...
const { getCliCommonPublicHealthCenterList } = storeToRefs(cliCommonStore)
...
const openAddModal = async (params = null) => {
  await mtUtils.smallPopup(UpdateCliCommonPublicHealthModal, {
...
const init = async () => {
  ...
  if (response) {
    cliCommonOptionList.value = getCliCommonPublicHealthCenterList.value.filter(
  ...
  <q-page :style="{ 'min-height': 'unset !important' }">
    ...
    <div class="row items-center justify-between q-px-lg tableBox">
      ...
      <div class="caption1 regular text-danger">
        <q-icon name="warning" />
        保健所院内用CDの重複禁止、マスタデータの為、責任者のみ編集可能です。
      </div>
    </div>
    <q-table
      :columns="columns"
