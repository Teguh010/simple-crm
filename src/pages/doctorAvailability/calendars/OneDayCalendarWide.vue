...
const calculateDynamicCellWidth = computed(() => {
  ...
  // Get maximum number of overlapping events
  ...
  // Calculate widths
  const totalEventsWidth = maxOverlappingEvents * MIN_EVENT_WIDTH
  const totalGapsWidth = (maxOverlappingEvents - 1) * EVENT_GAP
  const totalPaddingWidth = 2 * CONTAINER_PADDING
  
  // Add extra padding for safety
  const minColumnWidth = totalEventsWidth + totalGapsWidth + totalPaddingWidth + 32 // Extra padding
  ...
  // Use the larger of calculated widths
