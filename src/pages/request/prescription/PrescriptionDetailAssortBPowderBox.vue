...
const isGetFormCalled = ref(false)
...
const calculatedTotalDosage = computed(() => {
  if (prescriptionAssortForm.value.val_dosage_decided && props.calculatedTotalDosage)
    return (
      roundZeroAfterPoint(customRound(props.calculatedTotalDosage * Number(prescriptionAssortForm.value.val_dosage_decided)))
    )
})
...
watch(() => props.prescriptionAssort, (nowValue, oldValue) => {
  if (nowValue && nowValue.val_dosage_suggested != oldValue.old_dosage_suggested && !isGetFormCalled.value) {
    prescriptionAssortForm.value.val_dosage_decided = nowValue.val_dosage_suggested
  }
})
