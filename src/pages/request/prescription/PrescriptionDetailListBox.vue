...
const efficacyPetKg = computed(() => {
  ...
  return (
    ...
    (prescriptionStore.getPrescriptionHeaderByPet(
      prescriptionDetailForm.value.id_pet
    )?.val_weight / 1000)
  )
})
...
const calculateBookingDate = async () => {
  ...
  bookingDetail.value.id_customer = prescriptionStore.getPrescriptionHeaderByPet(prescriptionDetailForm.value.id_pet).id_customer
...
async function selectedTypeMedicineFormat(value) {
  ...
  if (prescriptionDetailForm.value.name_medicine_format == '粉A' ||
    ...
    prescriptionDetailForm.value.val_weight = prescriptionStore.getPrescriptionHeaderByPet(
      prescriptionDetailForm.value.id_pet
    ).val_weight
...
async function selectedTypeMedicine(value: any) {
  ...
  if (prescriptionDetailForm.value.isCurrentPH) {
    if (value == 2 && medicineObj.value.flg_dosage_variable) prescriptionDetailForm.value.type_medicine_dosage = value
    else if (value == 3 && medicineObj.value.flg_dosage_fixed) prescriptionDetailForm.value.type_medicine_dosage = value
    else if (value == 4 && medicineObj.value.flg_dosage_quantity) prescriptionDetailForm.value.type_medicine_dosage = value
    else prescriptionDetailForm.value.type_medicine_dosage = '4'
  }
  ...
  if (value == 4) {
    ...
    if (prescriptionDetailForm.value.isCurrentPH) {
      tempDosageFrequencyDosage = doseStore.getAllDoses.find((dosage: any) => dosage.value == prescriptionDetailForm.value.id_dosage_frequency)
    }
...
onMounted(async () => {
  ...
  if (medicineObj.value) {
    ...
    if (props.item.selectedTab == 1) {
      ...
      let typeMedicineDosage = props.item.type_medicine_dosage

      if (props.item && props.item.selectedTab == 1) {
        if (props.item.type_medicine_dosage == 2 && medicineObj.value.flg_dosage_variable) prescriptionDetailForm.value.type_medicine_dosage = props.item.type_medicine_dosage
        else if (props.item.type_medicine_dosage == 3 && medicineObj.value.flg_dosage_fixed) prescriptionDetailForm.value.type_medicine_dosage = props.item.type_medicine_dosage
        else if (props.item.type_medicine_dosage == 4 && medicineObj.value.flg_dosage_quantity) prescriptionDetailForm.value.type_medicine_dosage = props.item.type_medicine_dosage
        else prescriptionDetailForm.value.type_medicine_dosage = 4
        typeMedicineDosage = prescriptionDetailForm.value.type_medicine_dosage
      }
      
      if (props.item.type_medicine_dosage) {
        switch (typeMedicineDosage) {
          case 1:
      ...
      if (props.item.isCurrentPH) {
        ...
        if (response) {
          ...
          prescriptionDetailAssortListUI.map((pda) => {
            console.log(props.item.name_prescription_display, pda.id_prescription_detail_assort in props.item.pda_list, props.item.pda_list)
      ...
      <div v-show="prescriptionDetailForm.show">
        ...
        <div class="q-mx-md" v-if="!medicineObj.flg_drip_carrier">
          ...
          <div class="row q-col-gutter-md q-mb-md justify-between">
            ...
            <div class="col-auto">
              <q-btn v-if="isEffortAvailable" color="primary" class="text-white q-mr-md" size="sm" @click="()=>{
